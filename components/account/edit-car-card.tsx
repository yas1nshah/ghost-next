...
const CarCard = (params : any) => {
    ...
    return (
     <>
          <Link key={car.id} href={``}  className={`bg-card ${car.featured && "border border-primary"}  h-28 mx-0 md:h-40 md:px-2 w-full flex rounded-xl dark:text-white text-nowrap my-4 `}>
          ...
          <div className='flex gap-2 justify-end pb-4'>
            ...
            <Button
             ...
             variant={'outline'} size={'xs'}>{car.featured || featured ? "Featured": "Feature Car"}</Button>
          </div>
      </>    
